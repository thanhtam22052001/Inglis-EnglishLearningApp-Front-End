...
const LessonScreenGuessing = ({navigation, route}) => {
  useEffect(() => {
    const backAction = () => {
      //navigateBackward();
      return true;
  ...
  const navigateForward = () => {
    if (wordId > maxNumWords - 1)
      ...
      return navigation.push('LessonScreenGuessing', {
        listlesson: listlesson,
        indexlesson: indexlesson,
        wordId: wordId + 1,
  ...
  const changeSuggestion = () =>
    setSuggestion(
      (correct.value != true ? suggestion == 1 : suggestion == 3)
        ? 0
        : suggestion + 1,
    );
  return (
...
export default LessonScreenGuessing;
